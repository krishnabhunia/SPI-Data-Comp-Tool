...
namespace SPI_Data_Comp_Tool
{
    public partial class Form1 : Form
    {
        ...
        private bool boolOracle, boolSQL;
        ...
        OpenFileDialogData ofdd;// = new opOpenFileDialogData();
        ...
        private void columns_to_Ignore()
        {
            try
            {
                ...
                //    {
                //        cbL_columns.SetItemChecked(cbL_columns.FindStringExact(dc.ColumnName.ToString()), true);
                //        lblExpressionStatus.Text = "Some of columns are ignored";
                //    }
                //}
                ...
                foreach (var item in cic_ReferenceID)
                {
                    ...
                    //if(!boolCheck)
                    //{
                    //    str_col_to_include_in_comparison = str_col_to_include_in_comparison + item.ToString() + " , ";
                    //}
                }
                ...
                foreach (object i in cbL_columns.Items)
                {
                    ...
                    {
                        //if (i.ToString() != cb_referenceID.Text)
                        //if (!(clb_ReferenceID.GetItemChecked(clb_ReferenceID.FindStringExact(i.ToString()))))
                        //{
                        //lst_col_to_include_in_comparison.Add(i.ToString());
        ...
        private void btn_Connect_Reference_ID_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //cb_referenceID.DataSource = ds.Tables[0];
                //cb_referenceID.DisplayMember = ds.Tables[0].Columns.ToString();
                //cb_referenceID.ValueMember = ds.Tables[0].Columns.ToString();

                if (boolStatus)
                {
                    foreach (DataColumn dc in ds.Tables[0].Columns)
                    {
                        //cb_referenceID.Items.Add(dc.ToString());

                        clb_ReferenceID.Items.Add(dc.ToString());
                        cbL_columns.Items.Add(dc.ToString());
                    }

                    //if (cb_referenceID.Items.Count > 0)
                    //{
                    //    cb_referenceID.SelectedIndex = 0;
                    //}
                }
        ...
        private void btn_Set_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                CheckedListBox.ObjectCollection cic_oc_target = cbL_TargetColumns.Items;
                ...
                if (clb_ReferenceID.CheckedItems.Count > 0)
                {
                    boolStatus = true;
                    tb_FileName.Text = tb_Source_tableName.Text + "_output";
                    toggleCondition(true);
        ...
        private void backgroundWorker1_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                ...
                if (boolStatus)
                {
                    //ds = connect_Oracle_DB(str_col_to_include_in_comparison);
                    //ds = connect_Oracle_DB();
                ...
                if (boolStatus)
                {
                    //ds = connect_SQL_DB(str_col_to_include_in_comparison);
                    //ds = connect_SQL_DB();
        ...
        private void clb_ReferenceID_ItemCheck(object sender, ItemCheckEventArgs e)
        {
            //try
            //{
            //    cbL_columns.SetItemChecked(e.Index, clb_ReferenceID.GetItemChecked(e.Index));
            //}
            //catch (Exception ex)
            //{
            //    myException_BG = ex;
        ...
        //    public Excel(string strPath, int Sheet)
        //    {
